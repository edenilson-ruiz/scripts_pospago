...
set @periodo_corte		= '202004'
set @periodo_perma_f1	= '202004'
...
--Provision vs Real
--Provision = 24635.0645132726 | 26/11/2019
--Real		= 24998.8315044231 | 06/12/2019
--select SUM(comision_sin_iva) comision from DM_COMISV_POSPAGO.dbo.PagoPospagoPermanenciaDEXMasivoSM where PERIODO_PAGO = '201912'

if @acreedor_id = ''
 begin
	DELETE DM_COMISV_POSPAGO.dbo.PagoPospagoPermanenciaDEXMasivoSM WHERE PERIODO_PAGO = @periodo_pago
	
	
	/*--------------------------------------------------------------------------------------------------
	PERMANENCIA 1, DESDE 201902, NO OLVIDAR COLOCAR LAS OTRAS TRANSACCIONES EL OTRO MES.
	--------------------------------------------------------------------------------------------------*/
	INSERT INTO DM_COMISV_POSPAGO.dbo.PagoPospagoPermanenciaDEXMasivoSM
	SELECT 
		Y.*
		,CASE
		...
		,getdate() FECHA_PROCESO --INTO PagoPospagoPermanenciaDEXMasivoSM
	FROM (
		SELECT 
		...
		SELECT A.PERIODO
			...
			,CASE
				WHEN ISNULL(B.FACT01,0) >= 1 AND B.CO_ESTADO_ACT = 'a' THEN 'SI'
				ELSE 'NO'
			 END APLICA_PERMA
			,1 UNIDAD_GLOBAL	
			,CASE
				WHEN ISNULL(B.FACT01,0)  >= 1 AND B.CO_ESTADO_ACT = 'a' THEN 1
				ELSE 0
 ...
 begin
	DELETE DM_COMISV_POSPAGO.dbo.PagoPospagoPermanenciaDEXMasivoSM WHERE PERIODO_PAGO = @periodo_pago and ACREEDOR_ID = @acreedor_id
 ...
 select PERIODO_PAGO, 'REAL' ESCENARIO, TRANSACCION, COUNT(*) CANT, SUM(comision_sin_iva) PERMA 
 from DM_COMISV_POSPAGO.dbo.PagoPospagoPermanenciaDEXMasivo
 where PERIODO_PAGO >= '201909'
 group by PERIODO_PAGO, TRANSACCION
 UNION ALL 
 select PERIODO_PAGO, 'PROVISION' ESCENARIO, TRANSACCION, COUNT(*) CANT, SUM(comision_sin_iva) PERMA 
 from DM_COMISV_POSPAGO.dbo.PagoPospagoPermanenciaDEXMasivoSM 
 group by PERIODO_PAGO, TRANSACCION
 order by PERIODO_PAGO
 
 
 --select * from DimMetasDEXCorpoRevenue where PERIODO = '202001' and FACT_MOVIL_NUEVO = 1700
