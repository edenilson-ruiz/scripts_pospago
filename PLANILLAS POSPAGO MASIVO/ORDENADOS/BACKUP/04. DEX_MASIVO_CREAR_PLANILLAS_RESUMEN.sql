...
if @acreedor_id = ''
	begin
		...
		SELECT X.PERIODO_PAGO
			...
			,CONVERT(VARCHAR,X.NEGOCIO_ID) + '-' + X.ACREEDOR_ID + '-DEXC-' +  CONVERT(varchar(8),dateadd(d,-(day(getdate()-1)),getdate()),112) CODIGO_PLANILLA
		FROM (
		SELECT PERIODO_PAGO
 ...
 */
 ...
 select SUM(COMISION) from DM_COMISV_POSPAGO.dbo.PlanillasPospagoResumenMasivo where PERIODO_PAGO = '201812' and DISTRIBUIDOR like '%TDM%'
 
 --5056.85398230089
 
 select * from DM_COMISV_POSPAGO.dbo.PlanillasPospagoResumenMasivo where PERIODO_PAGO = '201812' and DISTRIBUIDOR like '%TDM%'
 
 
 
 select * from PlanillasPospagoResumenCorporativo where codigo_planilla = '1-700000047-CORP-20181204'
 
 
 select * from DimDealer where dealer_code in (
'DVCES.DC11-04641485-7',
'DVCES.DC16-003607267'
 )
 
 
 
 select * from FactPospagoVentas where dn_num in (
 '50370694481',
'50370693309'
 )
