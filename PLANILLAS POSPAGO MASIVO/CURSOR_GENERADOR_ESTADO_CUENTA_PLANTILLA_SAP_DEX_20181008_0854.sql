...
SELECT A.PERIODO_PAGO
	...
	,A.ACREEDOR_ID
	,A.DISTRIBUIDOR
FROM [DM_COMISV_POSPAGO].[dbo].[PlanillasPospagoResumenMasivo] a
WHERE a.PERIODO_PAGO = '201810'
	AND A.COMISION >= 0
	--AND A.DISTRIBUIDOR LIKE '%CRECE%'
	--and A.ACREEDOR_ID = '700000134'
...
OPEN cur_pospago_dex
	...
	FETCH cur_pospago_dex INTO @DEX_PERIODO_PAGO, @DEX_CANAL, @DEX_NEGOCIO, @DEX_ACREEDOR, @DEX_ACREEDOR_NOMBRE
	...
	WHILE (@@FETCH_STATUS = 0)
		BEGIN
			...
			set @total_descuentos = ISNULL(( SELECT SUM(A.COMISION)*-1 
									  FROM PlanillasPospagoResumenMasivo A
