...
begin
	...
	DELETE FROM PagoPospagoRenovacionesDEXCorpoRecalculo WHERE PERIODO = @periodo AND DISTRIBUIDOR = @NombreDistribuidor
	
	--DROP TABLE PagoPospagoRenovacionesDEXCorpo

	INSERT INTO PagoPospagoRenovacionesDEXCorpoRecalculo
	SELECT X.*	
		,1 UNIDAD
		,CASE
			WHEN EXISTS (SELECT * 
		...
		,CASE 
			...
			ELSE 0
		 END PRORRATEO
		,getdate() FECHA_RECALCULO --INTO PagoPospagoRenovacionesDEXCorpoRecalculo
	FROM (
