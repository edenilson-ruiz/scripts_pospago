...
if @acreedor_id = ''
	begin 
		...
		SELECT X.PERIODO_PAGO
			...
			,CONVERT(VARCHAR,X.NEGOCIO_ID) + '-'+ X.ACREEDOR_ID + '-CORP-' +  CONVERT(varchar(8),dateadd(d,-(day(getdate()-4)),getdate()),112) CODIGO_PLANILLA
		FROM (
		SELECT PERIODO_PAGO
		...
		SELECT PERIODO_DESCUENTO AS PERIODO_PAGO
			...
			,0 META_VENTA
			,0 ALCANCE_META_VENTA
			,SUM(A.MONTO_COMISION)*-1 COMISION
			,B.CUENTA_SAP
			,B.CENTRO_COSTO_SAP
			,B.NEGOCIO_ID
			,C.NEGOCIO_NOMBRE
...
where a.PERIODO_PAGO = '201811'
 ...
 /*
 SELECT * FROM DM_COMISV_POSPAGO.dbo.PlanillasPospagoResumenCorporativo WHERE PERIODO_PAGO = '201811' and distribuidor like '%SIGNO%' 
