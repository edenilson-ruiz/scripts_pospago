...
set @p_periodo_recalculo	= '202002'
set @p_distribuidor			= 'SIGNO'

-- diferencias desde recalculo hacia tabla de pago
 ...
 */
 ...
 SELECT A.[TICKLER_NUMBER]
      ...
      ,A.[NEGOCIO_NOMBRE]
      ,A.[PRORRATEO]   
FROM [DM_COMISV_POSPAGO].[dbo].[PagoPospagoRenovacionesDEXCorpoRecalculo] A
	left JOIN [DM_COMISV_POSPAGO].[dbo].[PagoPospagoRenovacionesDEXCorpo] B ON B.CO_ID = A.CO_ID AND B.PERIODO = A.PERIODO and B.DISTRIBUIDOR = A.DISTRIBUIDOR	
WHERE A.PERIODO = @p_periodo_recalculo
	AND A.DISTRIBUIDOR = @p_distribuidor
	AND isnull(A.COMISION_SIN_IVA,1) != isnull(B.COMISION_SIN_IVA,0)
	and A.CO_ID in ('11701256',
'11701242',
'12895263')
