...
--select * from DimAcreedor where acreedor_nombre_comercial like '%ACC%'
...
if @acreedor_id = ''
	begin
		...
		SELECT X.PERIODO_PAGO
			...
			,CONVERT(VARCHAR,X.NEGOCIO_ID) + '-' + X.ACREEDOR_ID + '-DEXC-' +  X.PERIODO_PAGO + SUBSTRING(CONVERT(VARCHAR,GETDATE(),112),7,2) + '-' + REPLACE(convert(varchar, GETDATE(), 108),':','-') CODIGO_PLANILLA
		FROM (
		SELECT PERIODO_PAGO
